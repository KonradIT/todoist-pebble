...
{    
    try {
        ...
        {
             json.sort((a, b) => {
                 const d1 = new Date(a.due_date_utc);
                 const d2 = new Date(b.due_date_utc);
                 return d1 - d2;
            });   
        }
        else
...
function getProjectsFromToken()
{
    var token = localStorage.getItem("todoistMiniTokenV7");
    var url = "https://api.todoist.com/sync/v9/sync";
    var params = "sync_token=*&resource_types=[\"projects\"]";
    xhrRequest(url + "?" + params, 'GET', getProjects, token);
}
...
function getItemsForSelectedProject(projectID)
{
    ...
    var token = localStorage.getItem("todoistMiniTokenV7");
    var url = "https://api.todoist.com/sync/v9/sync";
    var params = "sync_token=*&resource_types=[\"items\"]";
    xhrRequest(url + "?" + params, 'GET', getItems, token);
}
...
function addNewItem(itemText, projectID)
{
    ...
    var token = localStorage.getItem("todoistMiniTokenV7");
    var url = "https://api.todoist.com/sync/v9/sync";
    var params = "commands=" + encodeURIComponent(JSON.stringify(commandsjson));
    xhrRequest(url + "?" + params, 'GET', addItem, token);
}
